...
void Gui::startRendering()
{
    ...
    while (fisch.isRunning)
    {
        ...
        if (shouldRender)
        {
            ...
            ImGui::Begin("Fisch Macro v0.1.0 (insert to show/hide)", &fisch.isRunning);
            {
                static int page{};
                ...
                ImGui::BeginChild("content", ImVec2(0, 0), true);
                {
                    ...
                    case 1:
                        static int index = std::find(config.data.configsString.begin(), config.data.configsString.end(), config.data.configTXTname) != config.data.configsString.end()
                            ? std::distance(config.data.configsString.begin(), std::find(config.data.configsString.begin(), config.data.configsString.end(), config.data.configTXTname))
                            : 0;
                        static int prevIndex = index;
                        ImGui::SetNextItemWidth(ImGui::GetWindowWidth() - 15);
                        ImGui::ListBox("", &index, config.data.configsCString.get(), config.data.configsString.size());
                        if (index != prevIndex)
                        {
                            prevIndex = index;
...
Gui::Gui()
{
    ...
    GetClientRect(fisch.robloxHWnd, &robloxClientRect);
    ClientToScreen(fisch.robloxHWnd, &robloxClientToScreenPoint);
    hWnd = CreateWindowExW(WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TOOLWINDOW/*| WS_EX_NOACTIVATE*/, wc.lpszClassName, L"fisch macro", WS_POPUP, robloxClientToScreenPoint.x, robloxClientToScreenPoint.y, robloxClientRect.right, robloxClientRect.bottom, nullptr, nullptr, wc.hInstance, this);
    if (!hWnd)
